...
function hoanthanhbtndatve(){
        ...
        var seatReturnway = ghenguoilonve.concat(ghetreemve);
        ...
        if(totalSeat > 10 && companyId !== 'TC0Ah1r7bCvzRlpF') {
            $.alert({
                title: 'Thông báo!',
                type: 'orange',
                typeAnimated: true,
        ...
        // if(response.length > 0) {
            ...
            else //Thanh toan ve khu hoi
            {
                ...
                var informationsBySeats = new Array();
                var informationsBySeatsReturn = new Array();
                ...
                $.each(seatReturn , function(index, val) { 
                    informationsBySeatsReturn.push({'seatId': val, 'fullName': fullName, 'phoneNumber': phoneNumber, 'isAdult': true, 'mealPrice': 0, 'insurancePrice': 0, "extraPrice": extraPrice,"agencyPrice": ticketPriceOneway + extraPrice,"isTransshipment": false,
                                            "pointUp": pointUpReturn, "pointDown": pointDownReturn, "promotionCode": $('#promotionCode').val(), "pickUpAddress" : pickUpAddress,  "dropOffAddress" : dropOffAddress
                                        })
                });
                ...
                var dataSendReturn = {
                    "tripId" : tripIdReturn,
                    "informationsBySeats": informationsBySeatsReturn,
                    "packageName": "vn.dobody.anvuicustomer",
                    "timeZone": 7,
                    "platform": 2
                }
                ...
                dataSend = removeItemNull(dataSend);
                dataSendReturn = removeItemNull(dataSendReturn);
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if(data.status != 'success' ) {
                            ...
                            console.log(dataSend,'thanhcong1');
                            console.log(ticketReturn,'thanhcong2');
                            console.log(dataSendReturn,'thanhcong3');
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    if(data.status != 'success') {
                                        ...
                                        console.log(ticketReturn,'mavetk');

                                        setTimeout(function() {
                                            if(paymentType == 1 ) {
                                            

                                            var a = "https://ticket-new-dot-dobody-anvui.appspot.com/interbuslines/dopay?vpc_OrderInfo=" + ticketReturn + "&vpc_Amount=" + (100 * totalMoneyAfter + ((totalMoneyAfter * 0.01)*100 + 1000*100)) + "&phoneNumber=" + phoneNumber + "&packageName=web";
                                            window.location.href = a
                                                ...
                                                var a = "https://ticket-new-dot-dobody-anvui.appspot.com/inter-payment/dopay?vpc_OrderInfo=" + ticketReturn + "&vpc_Amount=" + (100 * totalMoneyAfter + ((totalMoneyAfter * 0.01)*100 + 1000*100)) + "&phoneNumber=" + phoneNumber + "&packageName=web";
                                                window.location.href = a

                                            }else if(paymentType == 8){
                                                url = "http://mgp.anvui.vn/?ticketId="+ticketReturn+'&paymentType='+0;
                                                window.location.href = url;
                                            } else if(paymentType == 7){
                                                $("#bankcode").modal({
                                                    backdrop: 'static',
                                                ...
                                                $.ajax({
                                                  ...
                                                  success: function(result) {
                                                    ...
                                                    $("#hoanthanhQrCode").click(function(){
                                                        var bankCode =  $('.bankCode:checked').val();
                                                        var url = 'https://ticket-new-dot-dobody-anvui.appspot.com/vnp/pay?vnp_OrderInfo='+ticketReturn+'&packageName=web&bankcode='+bankCode+'&companyId='+companyId;
                                                        window.location.href = url;
                                                    })
                                                  },
                                                  error: function () {
...
function selectTripOneWay(trip) {
    ...
    $('#list-oneway .alltypecars').removeClass('trip-active');
    $('.dk-'+tripIdOneway).addClass('trip-active');
...
function selectTripRoundWay(trip) {
    ...
    tripStatus = $(trip).data('tripstatus');
    routeId = $(trip).data('route');
    pointDownIdReturn = $(trip).data('pointdownid');
    pointUpIdReturn = $(trip).data('pointupid');
    $('.intimeReturn').html(getFormattedDate(intimeReturn, 'time'));
    console.log(tripStatus);


    console.log(pointDownIdReturn,'pointDownIdReturn');
    console.log(pointUpIdReturn,'pointUpIdReturn');
    console.log(tripIdReturn,'tripIdReturn');

    var getForCustomer2 = getForCustomer(tripIdReturn, pointDownIdReturn, pointUpIdReturn);
    console.log(getForCustomer2);


    pointUpReturn = JSON.parse($(trip).find('.pointUp').text());
    pointDownReturn = JSON.parse($(trip).find('.pointDown').text());


    $('#list-roundway .alltypecars').removeClass('trip-active');
    $('.dk-'+tripIdReturn).addClass('trip-active');
        ...
        for (var floor = 1; floor < addSeatMap.numberOfFloors + 1; floor++) {
                ...
                for (var row = 1; row < addSeatMap.numberOfRows + 1; row++) {
                    ...
                    for (var column = 1; column < addSeatMap.numberOfColumns + 1; column++) {
                        ...
                        coghe = false;
                        iddd = '';
                        seatInfoReturn = addSeatMap.seatList;// lay du lieu seatMap
                        $.each(addSeatMap.seatList, function (index, val) {
                            var id = val['seatId'];
                            var id1 = ignoreSpaces(id.replace(',', '_' , ' '));
                            ...
                            if(val['floor'] != floor || val['row'] != row || val['column'] != column) {
                                ...
                                if (val['seatType'] == 2) {
                                    ...
                                    html += '<div class="col-md-2 col-sm-2 col-xs-2 ghe-' + addSeatMap.numberOfColumns + '">' +
                                        '<div class="chonghe ghetrong '+id+'" id="chongheve_' + id1 + '" onclick="chongheve(\'' + id + '\')" data-over="' + val['overTime'] + '">' +
                                        '</div></div>';
                                }
                            }
                        });
            ...
            $.each(getForCustomer2, function (index, val) {
                if(val.ticketStatus == 2 && val.overTime > Date.now()){
                    $("#chongheve_"+val.id).addClass("ghedaban");
                    $("#chongheve_"+val.id).removeAttr( "onclick" );
                }else if(val.ticketStatus == 3 || val.ticketStatus == 7 || val.ticketStatus == 4 || val.ticketStatus == 5 ){
                    $("#chongheve_"+val.id).addClass("ghedaban");
                    $("#chongheve_"+val.id).removeAttr( "onclick" );
                }
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $.each(result['trips'], function (key, item) {
                ...
                console.log(item.additionPriceForUserType);
                ...
                if(isBack) {
                    ...
                    var schedule = '<li class=" '+typeclass+ ' dk-'+item.tripId + ' scheduleId-'+item.tripId+item.pointUp.id+item.pointDown.id+' '+ daxuatben +  ' "  > <div class="container"> ';
                } else {
                    var additionPrice = item.additionPriceForUserType||{};
                    var schedule = '<li class=" '+typeclass+ ' dk-'+item.tripId + ' scheduleId-'+item.tripId+item.pointUp.id+item.pointDown.id+' '+ daxuatben + ' " >';
                }

                if(isBack) {
                    schedule += ' <div class="hidden-xs"> <div class="offer-container hidden-sm hidden-xs"><div class="container-fluid payoo-offer-container"> <div class="row" style="display:none;"> <div class="col-sm-4 col-xs-3 pd-0 mb10" style="width:auto"> <div class="offer-tag"></div></div><div class="col-sm-10 col-xs-10 pd-0 ml20" style="margin-top:2px;"> <span></span> <span class="noti-detail" style="cursor:pointer;color:#1867bc"> '+ item.vehicleTypeName +' </span> </div></div><div class="pop-vxr w300 payoo-offer-tip" style="display: none;"> <div class="body-pop"> <div> <p style="line-height: 20px;white-space: pre-line;"></p></div></div></div></div></div><div class="col-2 col-md-3 col-sm-3 col-xs-2"> <div class="row"> <div class="pull-left comp-name-container"> <h6>'+item.pointUp.name+' - '+item.pointDown.name +'</h6> </div></div></div><div class="col-4 col-md-6 col-lg-5 col-sm-3 col-xs-4 detail-route"> <div class="col-12 ml10" style="text-align:center;"> <div class="text-left-from"> <h6>'+ msToTime(item.startTime) +'</h6> </div><div class="text-left" style="padding:0;"> - </div><div class="text-left-to"> </div><div class="clearfix"></div><div class="pull-left hidden-sm" style="width:100%;"> <div class="col-5 col-md-5 col-sm-5"> <a></a> </div><div class="col-2 col-md-2 col-sm-2"></div><div class="col-5 col-md-5 col-sm-5 payoo-mt2"> <a></a> </div></div></div></div><div class="col-2 col-md-2 col-sm-3 col-xs-2" style="text-align:center"> <div style="margin-bottom:3px !important" ></div><div class="clearfix"></div><div class="service-call seats-text mt15" style=""> <span>' +item.totalEmptySeat+ '</span> Ghế trống </div> <div class="service-call seats-text mt15" style=""> <span> ' +item.seatMap.seatMapName+ '</span>  </div></div><div class="text-right pr0 col-md-3 col-sm-3 col-xs-2 col-lg-2"><h6 style="font-weight: bold;"> '+ ticketPrice2 +' <small style="vertical-align: top;" class="unit-price-small">đ</small> </h6> <p style="margin:0;"> <div class="ticket-ac-btn btn-vxr-lg btn pull-right w100 hasSeat closed " onclick="selectTripRoundWay(this)" ' +
                        'data-ratio="' + 1 + '" ' +
...
function getForCustomer(tripIdData,pointUpId,pointDownId ){
