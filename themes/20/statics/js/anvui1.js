...
function hoanthanhbtndatve(){
        ...
        if($('#phoneNumber').val() == '') {
            ...
            if(isRound == 0){
                ...
                $.ajax({
                    type: 'POST',
                    url: 'https://interbuslines.com/dat-ve?sub=order',
                    data: dataSend,
                    success: function (data) {
                        ...
                        if(data.code != 200) {
                            ...
                            if(paymentType == 1) {
                                ...
                                var a = "https://dobody-anvui.appspot.com/interbuslines/dopay?vpc_OrderInfo=" + data.results.ticket.ticketId + "&vpc_Amount=" + (100 * totalMoneyAfter + ((totalMoneyAfter * 0.01)*100 + 1000*100)) + "&phoneNumber=" + phoneNumber + "&packageName=web";
                        ...
                        if(data.code != 200) {
                            ...
                            $.ajax({
                                ...
                                data: {
                                    ...
                                    "needPickUpHome":"true",
                                    "dropOffAddress":"",
                                    "getOffTimePlan":ticketPriceReturn
                                },
                                success: function (data) {
                                    if(data.code != 200) {
                                        $.alert({
...
function selectTripOneWay(trip) {
    ...
    var appendDatve = '<div id="select-seat-oneway">';
        appendDatve +='<div class="col-md-12 wp-seatr">';
        appendDatve +='<div class="row">';
        appendDatve +='<div class="col-md-4">';
...
function selectTripRoundWay(trip) {
    ...
    var appendDatve = '<div id="select-seat-oneway">';
        appendDatve +='<div class="col-md-12 wp-seatr">';
        appendDatve +='<div class="row">';
        appendDatve +='<div class="col-md-4 no-padding-right">';
        appendDatve +='<div class="select-seat" id="chooseRound">';
        ...
        appendDatve +='</div>';
        appendDatve +='</div>';
        appendDatve +='<div class="col-md-4 no-padding">';
    ...
    $('#list-roundway>li').removeClass('trip-active');
    $("#list-roundway>li #select-seat-oneway").remove();
    $(this).parent(".alltypecars").addClass('trip-active');
    ...
    offPointReturn = $(trip).data('getoffpoint');
    startDateReturn = $(trip).data('startdate');
    startTimeReturn = $(trip).data('starttime');
    tripStatus = $(trip).data('tripstatus');
    routeId = $(trip).data('route');
    ...
    // $('.rv-tran-from').text(getInPointName);
    // $('.rv-tran-to').text(getOffPointName);
    if(tripStatus == 2 || startTimeReturn < timeNow && startDateReturn <= Now) {
        $.alert({
            title: 'Cảnh báo!',
            type: 'red',
            typeAnimated: true,
        ...
        $('#select-seat').prop('disabled', false);
        $(".scheduleId-"+scheduleIdReturn).append(appendDatve);
        
        
        //chọn trip
        $.getJSON("https://interbuslines.com//dat-ve?tripId=" + tripIdReturn + '&scheduleId=' + scheduleIdReturn, function (data) {

            var html = '';
            //Theo số tầng
            for (var floor = 1; floor < data.seatMap.numberOfFloors + 1; floor++) {
                if(floor == 1){
                    html += '<div class="col-md-12 col-sm-12 col-xs-12 tachtang dz-'+data.seatMap.numberOfColumns+'">';
                } else {
                    html += '<div class="col-md-12 col-sm-12 col-xs-12 dz-'+data.seatMap.numberOfColumns+'">';
                }
