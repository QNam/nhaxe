$(document).ready(function() {
    ...
    $('#bookingWaterBus #schedule-list').hide();
    ...
    //     dataType: "json",
    //     data: JSON.stringify({ packageName: "vn.anvui.saigonwaterbus", companyId: "TC03j1IanqGKIUS"}),
    ...
    function payment(dataPayment, dataPaymentReturn, ticketId) {
        if(dataPayment !== undefined && dataPayment !== '') {
            $.ajax({
                type: "POST",
                url: "https://www.anvui.vn/createnoseatid",
                data: dataPayment,
                success: function (result) {
    ...
    function oncePay(ticketId){
        var url = "https://dobody-anvui.appspot.com/onepay/pay?vpc_OrderInfo="+ticketId+"&scope=0&companyId=TC03j1IanqGKIUS&packageName=web";
        window.location.href = url;
    }
});
