...
$(document).ready(function () {
    ...
    $.ajax({
        type: "POST",
        url: "https://www.anvui.vn/chuyenAV",
        data: {
            idAV: companyId
        },
    ...
    $('#hoanthanhbtn').click(function () {
        ...
        var email = $('#email').val()||'';
        var note = $('#note').val()||'';
        var ngaydi = $('#depatureDate').val();
        var giodi = $('li').find('.intime').text();
        ...
        var txtTransshipment = '';
        if($('#transshipmentInPointOneway').val()!==''){
            txtTransshipment =$('#transshipmentInPointOneway option:selected').text();
        }
        ...
        if(isRound == 0){
            ...
            $.confirm({
                ...
                content: '<div>'+routeName+'</div>' +
                '<div>Ngày đi : '+ngaydi+' </div>'+
                '<div>Giờ xuất phát : '+giodi+' </div>'+
                '<div>Họ tên: <b>' + fullName + '</b></div>' +
                ...
                '<div>Đón : '+txtTransshipment+'</div>'+
                    '<hr>'+
                '<div>Thành tiền : <b><span style="color: #fe5815;font-size: 15px">'+$('#totalMoneyOneway').text()+'</span></b></div>'+
                '<div>Khuyến Mại : <b><span style="color: #fe5815;font-size: 15px">'+($('#promotionPrice').text()||'0 VND')+'</span></b></div>'+
                '<div>Tổng tiền : <b><span style="color: #fe5815;font-size: 15px">'+$('#totalMoney').text()+'</span></b></div>',
                buttons: {
                    cancel: {
                        text: 'Hủy'
                    },
                                ...
                                success: function (data) {
                                    if(data.code != 200) {
                                        ...
                                        if(paymentType == 1) {
                                            ...
                                            setTimeout(function () {
                                                url = "https://dobody-anvui.appspot.com/onepay/pay?vpc_OrderInfo="+data.results.ticket.ticketId+"&scope=1&companyId="+companyId+"&packageName=web";
...
function selectTripOneWay(trip) {
    ...
    $('#transshipmentInPointOneway').html(tsmip);
    ...
    $('#transshipmentOffPointOneway').html(tsmop);
    ...
    if(tripStatus == 2) {
        ...
        $.getJSON("http://demo.nhaxe.vn/dat-ve?tripId=" + tripIdOneway + '&scheduleId=' + scheduleIdOneway, function (data) {
            ...
            for (var floor = 1; floor < data.seatMap.numberOfFloors + 1; floor++) {
                if (floor == 1) {
                    html += '<div class="col-md-6 col-sm-12 col-xs-12 tachtang border-full">';
                } else {
                    html += '<div class="col-md-6 col-sm-12 col-xs-12 border-full">';
                }

                html += '<div class="col-md-12 col-sm-12 col-xs-12 text-center"><strong style="font-size: 15px">Tầng ' + floor + '</strong></div>';
                    ...
                    for (var column = 1; column < data.seatMap.numberOfColumns + 1; column++) {
                        ...
                        $.each(data.seatMap.seatList, function (index, val) {
                            ...
                            if (val['floor'] != floor || val['row'] != row || val['column'] != column) {
                                ...
                                //Type = 2 là tài
                                if (val['seatType'] == 2) {
                                    html += '<div class="col-md-2 col-sm-2 col-xs-2 ghe-' + data.seatMap.numberOfColumns + '"><div class="chonghe driver" style="font-weight: bold">'+id+'</div></div>';
                                } else if(val['seatType'] == 1){
                                    html += '<div class="col-md-2 col-sm-2 col-xs-2 ghe-' + data.seatMap.numberOfColumns + '"><div class="chonghe door"></div></div>';
                                } else if (val['seatType'] == 5 || val['seatType'] == 6) { // Lần lượt là cửa cửa, Wc, phụ
                                    ...
                                    html += '<div class="col-md-2 col-sm-2 col-xs-2 ghe-' + data.seatMap.numberOfColumns + '"><div class="chonghe ghedaban">'+id+'</div></div>';
                                } else {
                                    html += '<div class="col-md-2 col-sm-2 col-xs-2 ghe-' + data.seatMap.numberOfColumns + '">' +
                                        '<div class="chonghe ghetrong" id="chonghe_' + id1 + '" onclick="chonghe(\'' + id + '\')" data-over="' + val['overTime'] + '">' +id+
                                        '</div></div>';
                                }
                            }
                        });
...
function selectTripRoundWay(trip) {
    ...
    $.each(transshipmentInPointReturn, function (k, v) {
        tsmipr += '<option data-address="' + v.address + '" ' +
            'data-price="' + v.transshipmentPrice + '" ' +
            'data-lat="' + v.latitude + '" ' +
            'data-long="' + v.longitude + '" ' +
            'value="' + v.pointId + '">' + v.pointName + '</option>';
    });
    $('#transshipmentInPointReturn').html(tsmipr);

    $.each(transshipmentOffPointReturn, function (k, v) {
        tsmopr += '<option data-address="' + v.address + '" ' +
            'data-price="' + v.transshipmentPrice + '" ' +
            'data-lat="' + v.latitude + '" ' +
            'data-long="' + v.longitude + '" ' +
            'value="' + v.pointId + '">' + v.pointName + '</option>';
    });
    $('#transshipmentOffPointReturn').html(tsmopr);
    ...
    if(!isBack) {
        ...
        if(isBaby) {
            if(ghetreemve.length > checkBabyReturnLength) {
                if(typeof addPri.amount !== "undefined"){
                    if(addPri.type===2){
                        ticketPriceReturn = ticketPriceReturn + addPri.mode * addPri.amount * ticketPriceReturn;
                    }else{
                        ticketPriceReturn = ticketPriceReturn + addPri.mode * addPri.amount;
                    }
                }
...
function getSchedule(startPoint, endPoint, date, routeId, isBack) {
    ...
    $.ajax({
        type: "POST",
        url: "https://www.anvui.vn/listSchedule",
        data: {
        ...
        success: function (result) {
            ...
            $.each(result, function (key, item) {
                ...
                console.log(item.additionPrice);

                schedule += '<td style="display: none" class="additonPriceInfo">'+JSON.stringify(additionPrice)+'</td>';
                ...
                schedule += '<td class="table-price"><strong>'+ ticketPrice2 +' VND</strong></td>';
                schedule += '</tr>';
                if(isBack) {
