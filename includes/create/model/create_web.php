...
class CreateWeb {
    ...
    public function registeranvui() {
        ...
        if($this->r->get_string('customerId', 'POST') != ''){
            $d['customerId'] = $this->r->get_string('customerId', 'POST');
        }

        // $d['TRANSPORT_COMPANY_SERVICE_PACK_ID_NULL'] = 'SP00000002';
        $d['servicePackId'] = 'SP05K1yc46wZw9U';
        ...
        $rt = $this->PostAnvui('https://ticket-new-dot-dobody-anvui.appspot.com/company/create',$d);

        

        

        if($rt['code'] == 200 ||  $rt['code'] == 201){
            $data = array();
            $data['status'] = true;
            ...
            if ( $this->addWeb($rt['results']['User']['id']) ) {
                return $data;
            } else {
                $data['status']  = false;
